...
func NewWhereBuilder(dt DbType) *WhereBuilder {
	obj := &WhereBuilder{
		dbType: dt,
...
func (w *WhereBuilder) Eq(field string, arg interface{}) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.And(warpField(gd, field)+" = "+placeholders(), arg)
}

func (w *WhereBuilder) OrEq(field string, arg interface{}) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.Or(warpField(gd, field)+" = "+placeholders(), arg)
}

func (w *WhereBuilder) IsNull(field string) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.And(warpField(gd, field) + " IS NULL")
}

func (w *WhereBuilder) OrIsNull(field string) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.Or(warpField(gd, field) + " IS NULL")
}
...
func (w *WhereBuilder) Between(field string, arg1, arg2 interface{}) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.And(warpField(gd, field)+" (BETWEEN "+placeholders()+" AND "+placeholders()+")", arg1, arg2)
}

func (w *WhereBuilder) OrBetween(field string, arg1, arg2 interface{}) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.Or(warpField(gd, field)+" (BETWEEN "+placeholders()+" AND "+placeholders()+")", arg1, arg2)
}

func (w *WhereBuilder) In(field string, args ...interface{}) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.And(warpField(gd, field)+" IN ("+placeholders(len(args))+")", args...)
}

func (w *WhereBuilder) OrIn(field string, args ...interface{}) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.Or(warpField(gd, field)+" IN ("+placeholders(len(args))+")", args...)
}

func (w *WhereBuilder) LikeLeft(field string, arg string) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.And(warpField(gd, field)+" LIKE "+placeholders(), "%"+EscapeLike(arg))
}

func (w *WhereBuilder) OrLikeLeft(field string, arg string) *WhereBuilder {
	gd := getDialect(w.dbType)
	return w.Or(warpField(gd, field)+" LIKE "+placeholders(), "%"+EscapeLike(arg))
}
...
func (w *WhereBuilder) GetSqlStr() string {
	return NewParsePlaceholder(w.dbType, w.finalBuf.String(), w.args...).Parse().Result()
}

func (w *WhereBuilder) GetSql2Args() (string, []interface{}) {
	return NewParsePlaceholder(w.dbType, w.finalBuf.String(), w.args...).Replace().Result(), w.args
}
