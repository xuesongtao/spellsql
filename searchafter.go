...
type SearchAfter struct {
	SqlStr   any                  // 查询 base sql, sqlStr 支持 string/*builder.Select, 只能包含到 where 部分, 注: 查询部分, 必须包含 names 里的字段
	...
	Values   []any                // 分页值, 每次处理完后, 会自动根据查询结果里的值更新为最后一行的值, 以便下一次查询, 根据 大于 的条件进行查询
	OrderBys []string                     // 按什么进行排序, 默认: id asc, 例如: ["id ASC", "name DESC"], 如果不传, 则默认按 names 里的字段进行升序排序
	Size     int                          // 每次处理多少
	Dest     any                  // scan 对象, 即回调里的对象
	RowFn    func(_row any) error // 每行的回调函数
...
func (s *SearchAfter) initValues(row any) error {
...
func (w *SearchResults) Append(v any) *SearchResults {
...
func (w *SearchResults) LenEqual2Do(l int, f func(res []any) error, needReset ...bool) error {
...
func (w *SearchResults) LenGte2Do(l int, f func(res []any) error, needReset ...bool) error {
	defaultNeedReset := true
	if len(needReset) > 0 {
		defaultNeedReset = needReset[0]
	}
...
func (w *SearchResults) do(f func(res []any) error, needReset bool) error {
...
func (w *SearchResults) End2Do(f func(res []any) error) error {
	if w.Empty() {
		return nil
	}
	return f(w.data)
